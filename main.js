//var siteName = "benbenbob1.github.io";
	...
	function evaluate(str) {
		...
		if (cmd == "test") {
			...
			result = contactCommand(args);
		} else if (cmd == "clear") {
			result = clearCommand(args);
		} else if (cmd == "stuff") {
			result = stuffCommand(args);
		} else if (cmd == "skills") {
			result = skillsCommand(args);
		} else if (cmd == "skillz") {
			result = skillzCommand(args);
		} else if (cmd == "style") {
			result = styleCommand(args);
		} else if (cmd == "resume") {
			result = showResume();
		} else if (cmd == "cat") {
			result = catCommand(args);
		//} else if (cmd == "color") {
		//	result = colorCommand(args);
		} else if (cmd == "history") {
			result = historyCommand(args);
		}  else if (cmd == "about") {
			result = aboutCommand(args);
		} else if (cmd == "me") { 
			result = "\n‘"+htmlImage()+"’";
		} else {
